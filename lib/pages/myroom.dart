...
class _MyRoomState extends State<MyRoom> {
  ...
  Widget build(BuildContext context) {
    BackgroundProvider backgroundProvider =
        Provider.of<BackgroundProvider>(context);
    BackgroundSettingProvider backgroundSettingProvider =
        Provider.of<BackgroundSettingProvider>(context);
    if (backgroundProvider.isImage == false &&
        backgroundProvider.videoController.value.isInitialized) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          //영상으로 선택되어있을 땐 이걸 튼다.
          else if (!backgroundProvider.isImage &&
              backgroundProvider.videoController.value.isInitialized)
            SizedBox.expand(
              child: FittedBox(
                fit: BoxFit.cover,
          ...
          Container(
            child: backgroundSettingProvider.audioSpectrumEnabled
                ? AudioSpectrumWidget(
                    audioFilePath: 'audios/nature/defaultMainMusic.mp3')
                : null,
          )
        ],
      ),
